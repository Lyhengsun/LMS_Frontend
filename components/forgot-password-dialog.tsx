...
import { Mail, ArrowLeft, CheckCircle, Loader2, Shield, Clock } from 'lucide-react';
...
export const ForgotPasswordDialog = ({ open, onOpenChange }: ForgotPasswordDialogProps) => {
  ...
  const handleSendOtp = async (e: React.FormEvent) => {
    ...
    const userExists = demoEmails.includes(email.toLowerCase()) || 
                      approvedUsers.some((user: any) => user.email.toLowerCase() === email.toLowerCase());
  ...
  const handleResetPassword = async (e: React.FormEvent) => {
    ...
    if (password !== confirmPassword) {
      toast({
        ...
        description: "New password and confirm password don't match. Please try again.",
        variant: "destructive"
      });
      return;
    }
    ...
    const approvedUsers = JSON.parse(localStorage.getItem('approvedUsers') || '[]');
    const userIndex = approvedUsers.findIndex((user: any) => user.email.toLowerCase() === email.toLowerCase());
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-md">
        <DialogHeader>
          ...
          <DialogDescription>
            {step === 'email' && 'Enter your email address to receive an OTP for password reset.'}
          ...
          <form onSubmit={handleVerifyOtp} className="space-y-4">
            <div className="space-y-2">
              ...
              <div className="flex items-center justify-center space-x-2 text-sm">
                ...
                <span className={timeLeft > 0 ? "text-gray-600" : "text-red-500"}>
